...
if (!process.env.JWT_SECRET) {
  // Provide a safe fallback for local development so the server can run
  // without forcing the developer to set an env var. In production this
  // should always be set and we warn loudly.
...
// Serve static assets but do NOT auto-serve index.html for root path.
// This prevents unauthenticated users from receiving `index.html` briefly
// and avoids a flicker before the root route performs authentication checks.
